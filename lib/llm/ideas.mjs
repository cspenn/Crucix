...
Output ONLY valid JSON array. Each object:
{
  ...
  try {
    const result = await provider.complete(systemPrompt, context, { maxTokens: 16384, timeout: 90000 });
    if (result.finishReason === 'length') {
      console.warn('[LLM Ideas] Response truncated — consider increasing maxTokens or disabling model thinking mode');
    }
...
function parseIdeasResponse(text) {
  ...
  // Strip reasoning/thinking tags (e.g. Nemotron, Qwen, DeepSeek)
